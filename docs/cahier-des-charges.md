...
- **Forme de la zone sélectionnable** : rectangle (par défaut), cercle, ou **polygone régulier/équilatéral** (triangle par défaut, nombre de côtés réglable pour obtenir carré, pentagone, hexagone, etc.) — pas uniquement une découpe rectangulaire. La forme choisie détermine le contour final de la carte (et donc du Layer 3/cadre), pas seulement la zone de requête des données.
...
- **Police et taille du texte configurables, indépendamment pour le titre et les coordonnées** : choix de la police d'écriture (parmi une liste de polices adaptées à la gravure/découpe laser — mono-trait ou pleines selon le rendu voulu) et de la graisse, séparément pour le titre et pour les coordonnées.
